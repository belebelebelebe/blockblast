...
let isAnimating = false; // New flag to prevent input during animations
...
async function clearFilledRowsAndColumns() {
    isAnimating = true; // Block input during animations
    ...
    // Flash animation for cleared rows and columns
    for (let i = 0; i < 3; i++) {
        ctx.save();
    ...
    // Fade-out animation for cleared cells
    let cellsToClear = [];
    placedBlocks.forEach((block, bIdx) => {
        for (let row = 0; row < block.shape.length; row++) {
            for (let col = 0; col < block.shape[row].length; col++) {
    ...
    if (movesSinceLastClear > 3 || comboLevel === 0) {
        ...
        ctx.fillText("Combo: " + comboLevel, 410, 160);
        ctx.fillText("+" + 10 * linesCleared * comboLevel, 410, 200);
        ctx.restore();
    ...
    isAnimating = false; // Re-enable input after animations
    redrawAll();
}
...
canvas.addEventListener("mousedown", function (e) {
    if (isDragging || isGameOver || isAnimating) return; // Block input during animations
